 ...
 */
public class AhoCorasickDoubleArrayTrie implements Serializable {
    /**
        ...
        for (int i = 0; i < text.length(); ++i) {
            ...
            if (hitArray != null) {
                for (int hit : hitArray) {
                    int begin = position - l[hit];
//                    if (position - begin == 1) {
//                        continue;
//                    }
                    positionOffsetList[begin].put(position - 1, v[hit]);
                }
            }
            ++position;
        }
     ...
     */
    public List<Hit<Double>> parseText(CharSequence text) {
        int position = 1;
        int currentState = 0;
        List<Hit<Double>> collectedEmits = new ArrayList<Hit<Double>>();
        for (int i = 0; i < text.length(); ++i) {
            currentState = getState(currentState, text.charAt(i));
     ...
     */
    private void storeEmits(int position, int currentState, List<Hit<Double>> collectedEmits) {
        int[] hitArray = output[currentState];
        if (hitArray != null) {
            for (int hit : hitArray) {
                collectedEmits.add(new Hit<Double>(position - l[hit], position, v[hit]));
            }
        }
    }
     ...
     */
    public void parseText(CharSequence text, IHit<Double> processor) {
        int position = 1;
        int currentState = 0;
        for (int i = 0; i < text.length(); ++i) {
            currentState = getState(currentState, text.charAt(i));
     ...
     * @param processor A processor which handles the output
     */
    public void parseText(CharSequence text, IHitCancellable<Double> processor) {
        int currentState = 0;
        for (int i = 0; i < text.length(); i++) {
     ...
     * @param processor A processor which handles the output
     */
    public void parseText(char[] text, IHit<Double> processor) {
        int position = 1;
        int currentState = 0;
        for (char c : text) {
     ...
     * @param processor A processor which handles the output
     */
    public void parseText(char[] text, IHitFull<Double> processor) {
        int position = 1;
        int currentState = 0;
        for (char c : text) {
            currentState = getState(currentState, c);
     ...
     */
    public Hit<Double> findFirst(String text) {
        int position = 1;
        int currentState = 0;
        for (int i = 0; i < text.length(); ++i) {
            currentState = getState(currentState, text.charAt(i));
            ...
            if (hitArray != null) {
                ...
                return new Hit<Double>(position - l[hitIndex], position, v[hitIndex]);
            }
            ++position;
        }
        return null;
    ...
    public interface IHit<Double> {
         ...
         */
        void hit(int begin, int end, Double value);
    }

    /**
     * Processor handles the output when hit a keyword, with more detail
     */
    public interface IHitFull<Double> {
        /**
         * Hit a keyword, you can use some code like text.substring(begin, end) to get the keyword
         *
         * @param begin the beginning index, inclusive.
         ...
         */
        void hit(int begin, int end, Double value, int index);
    }

    /**
     * Callback that allows to cancel the search process.
     */
    public interface IHitCancellable<Double> {
        /**
         * Hit a keyword, you can use some code like text.substring(begin, end) to get the keyword
         *
         * @param begin the beginning index, inclusive.
         ...
         */
        boolean hit(int begin, int end, Double value);
    }

    /**
     * A result output
     *
     * @param <Double> the value type
     */
    public static class Hit<Double> {
        /**
        ...
        public Hit(int begin, int end, Double value) {
            this.begin = begin;
            this.end = end;
            this.value = value;
        }
    ...
    private class Builder {
        ...
        public void build(Map<String, Double> map) {
            // 把值保存下来
            v = new Double[map.size()];
            map.values().toArray(v);
            l = new int[v.length];
